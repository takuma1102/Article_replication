...
# Use mutiple_hetero_dollar_plot and mutiple_hetero_number_plot function

mutiple_hetero_dollar_plot(anti_DCDH_s_mva_logav5_cov, anti_fect_s_mva_logav5,
                    anti_panel_numeric[[1]], anti_panel_numeric[[2]],
                    ylim = c(-1,1))

mutiple_hetero_dollar_plot(pro_DCDH_s_mva_logav5_cov, pro_fect_s_mva_logav5,
                    pro_panel_numeric[[1]], pro_panel_numeric[[2]],
                    ylim = c(-1.5,2))

mutiple_hetero_number_plot(anti_DCDH_s_mva_5_covnum, anti_fect_s_mva_num5,
                           anti_panel_numeric_num[[1]], anti_panel_numeric_num[[2]],
                           ylim = c(-0.12,0.16))

mutiple_hetero_number_plot(pro_DCDH_s_mva_5_covnum, pro_fect_s_mva_num5,
...
# Use combined_mutiple_hetero_plot function

combined_mutiple_hetero_plot(anti_DCDH_s_mva_logav5_cov, anti_fect_s_mva_logav5,
                           anti_panel_numeric[[1]], anti_panel_numeric[[2]],
                           ylim = c(-0.8,1.35),anti_DCDH_s_mva_5_covnum, anti_fect_s_mva_num5,
                           anti_panel_numeric_num[[1]], anti_panel_numeric_num[[2]],
                           ylim_num = c(-0.12,0.16))

combined_mutiple_hetero_plot(pro_DCDH_s_mva_logav5_cov, pro_fect_s_mva_logav5,
                             pro_panel_numeric[[1]], pro_panel_numeric[[2]],
                             ylim = c(-2.1,2.3),pro_DCDH_s_mva_5_covnum, pro_fect_s_mva_num5,
                             pro_panel_numeric_num[[1]], pro_panel_numeric_num[[2]],
                             ylim_num = c(-0.24,0.28))

combined_mutiple_hetero_plot(anti_DCDH_s_mva_logav5_cov_1824, anti_fect_s_mva_logav5_1824,
...
## 7A.4 Number Outcome Plot ----------------------------------------------------

# check of result objects

DCDH_s_mva_5_covnum
fect_s_mva_num5
pre_s_panelmatch_outcomeplus_number
PE.results_s_baseline_outcomeplus_number
...
df_mplot_imp_number <- tibble(
  Time     = fect_s_mva_num5$time,
  Estimate = fect_s_mva_num5$att,
  LB.CI    = fect_s_mva_num5$att.bound[,1],
  UB.CI    = fect_s_mva_num5$att.bound[,2]
) %>%
  mutate(
    method = "Imputation",
    window = ifelse(Time < 0, "Pre", "Post")
...
df_mplot_pmpre_number <- tibble(
  Time     = as.numeric(gsub("t", "", names(pre_s_panelmatch_outcomeplus_number$estimates))),
  Estimate = unname(pre_s_panelmatch_outcomeplus_number$estimates),
  SE       = unname(pre_s_panelmatch_outcomeplus_number$standard.errors)
) %>%
  mutate(
    LB.CI   = Estimate - qnorm(0.975)*SE,
    UB.CI   = Estimate + qnorm(0.975)*SE,
...
df_mplot_pmpost_number <- tibble(
  Time     = as.numeric(gsub("t", "", names(PE.results_s_baseline_outcomeplus_number$estimate))),
  Estimate = unname(PE.results_s_baseline_outcomeplus_number$estimate),
  SE       = unname(PE.results_s_baseline_outcomeplus_number$standard.error)
) %>%
  mutate(
    LB.CI   = Estimate - qnorm(0.975)*SE,
    UB.CI   = Estimate + qnorm(0.975)*SE,
...
df_mplot_DCDH_number <- tibble(
  Time     = DCDH_s_mva_5_covnum$plot$data$Time,
  Estimate = DCDH_s_mva_5_covnum$plot$data$Estimate,
  LB.CI    = DCDH_s_mva_5_covnum$plot$data$LB.CI,
  UB.CI    = DCDH_s_mva_5_covnum$plot$data$UB.CI
) %>%
  mutate(
    method = "DCDH",
    window = ifelse(Time < 0, "Pre", "Post")
...
all_mplot_number <- ggplot(all_df_mplot_number, aes(x = Time, y = Estimate,
  ...
  coord_cartesian(xlim = c(-6, 6), ylim = c(-0.25, 0.25)) +
...
mplot_box_style <- ggplot(combined_mplot_df,
  ...
  facetted_pos_scales(
    y = list(
      Outcome == "Effect on Logged Dollar Amount"    ~ scale_y_continuous(limits = c(-1.95, 2)),
      Outcome == "Effect on Number Count"   ~ scale_y_continuous(limits = c(-0.23, 0.23))
    )
  ) +
